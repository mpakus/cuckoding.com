...
- Product name: Cuckoding
- App module: `AgentDesk` / `AgentDeskWeb`
- UI: Phoenix LiveView (dark only)
- Store: SQLite via Ecto (`ecto_sqlite3`)
- Desktop: ExTauri (native shell around this Phoenix app)
- Coordination: built-in internal A2A Hub, exposed to providers through a local MCP surface (`hub_*`, `memory_*`, search)
- Search/memory: SQLite projection by default; XERJ optional
- Providers: Codex, Claude, Cursor (`agent acp` or `cursor agent acp` from Cursor.app), OpenCode, SDK JSONL, remote attach
- Git: isolated worktree per session, including repositories that have no commits yet

Read `AGENTS.md`, then the one spec in `docs/` that matches the change. Do not contradict `docs/DECISIONS.md`. Dev loop: `./bin/dev`. HTTP binds `127.0.0.1` only.
