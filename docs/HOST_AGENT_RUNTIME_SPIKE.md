...
Task 1006 resolved that retained failure by never exposing the real home. Task 1018 later replaced per-run login for saved agents with one app-owned account HOME while keeping `CURSOR_CONFIG_DIR` and `CLAUDE_CONFIG_DIR` run-owned. After a real login showed that the isolated HOME cannot discover the macOS default keychain, the adapter selected Cursor's native file credential store under that private account HOME. The adapter verifies an empty shared MCP file and refuses project Cursor CLI, sandbox, MCP, and plugin overrides. A fresh authenticated provider smoke remains release evidence, not an implementation prerequisite.
