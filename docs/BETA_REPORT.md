...
| Deterministic two-board isolation | source prerequisite passed; D03 pending | Both disposable boards overlapped at Development with distinct branches, worktrees, specification artifacts, ownership markers, and event correlation in `test/cuckoding/project_workflow_test.exs`. No real provider, process-group, port, or enrolled-app observation was made. |
...
| Current-app-code developer build | pass | 2026-09-21: `d7a6222` passed `bin/dev.build`; its sterile verifier passed startup, token/session, crash, safe-mode, and update/rollback checks. The separate signing drill is recorded below; neither step satisfies clean-Mac or enrollment-package acceptance. |
| Release CI source gate | passed; configuration blocked | [Then-current-main run 35684138654](https://github.com/mpakus/cuckoding.com/actions/runs/35684138654) on `734128f` passed pinned setup and `mix quality` (290 tests, 10 properties, zero failures). The value-free preflight reported five absent Apple certificate/notary secrets; import, packaging, and publication did not run. |
| Signed and notarized build identified | current app and ZIP; enrollment package pending | `d7a6222` passed the local build, Developer ID signing, Apple notarization, stapling, same-Mac quarantined ZIP verification, and sterile embedded-release checks. Its checksum-matched ZIP is staged for `qa`; that account has not launched it. No signed updater, complete release package, or clean-Mac install is accepted. CI still lacks the Apple certificate/password and App Store Connect notary key/ID/issuer secrets. |
...
| Previously exposed provider key rotated | stakeholder-confirmed; no secret inspection | On 2026-09-23 the stakeholder confirmed rotation in this task. The secret value was not read, logged, or copied; this attestation does not prove future provider execution or waive Echo's source-validation gate. |
...
| F-1003-003 | High | An existing delivery run stopped in Development before an agent session was recorded; its durable event says only `unexpected_failure`, so its exact cause cannot be recovered from that event | Read-only inspection of blocked task/run `01a0bb7c-f6d3-776c-bffe-7da4600ebe81` / `01a0bcd7-0e7d-7df7-8263-a71361f5de57` on 2026-09-21; no provider rerun | diagnostic gap closed in source; root cause and provider retest open | codex | Task 1026 adds a bounded, non-secret code location for future unexpected failures; it cannot backfill the old event. Reproduce or retest with an authorized provider after key rotation, then identify and fix the actual launch failure before beta acceptance. |
| F-1003-004 | High | A delivery Coding run was interrupted after five minutes although its saved stage budget was one hour | Local run event `process.timeout` followed by INT and exit 130; saved workflow snapshot and `WalkingSkeleton.request/4` compared on 2026-09-23 | source fix passed; provider retest open | codex | Task 0405 follow-up reads the snapshotted stage budget; focused regression and full source quality gate pass. The interrupted run and its uncommitted worktree were preserved. |
| F-1003-005 | High | Two delivery Review runs were rejected before model work because the nested `findings[].evidence` output schema was not closed | Redacted process logs show `invalid_json_schema` at that exact path; local run events show adapter exit 1 | source fix passed; provider retest open | codex | The shared Review schema now has a required `detail` field and `additionalProperties: false`; focused regression and full source quality gate pass. Historical runs were not rewritten. |
