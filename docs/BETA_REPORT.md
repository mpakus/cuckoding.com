...
| Signed and notarized build identified | current app only; enrollment package pending | Current `17d367a` source was signed, notarized, stapled, and same-Mac verified as a post-staple ZIP staged for the `qa` account. No signed updater, complete release package, QA-account launch, or clean-Mac install is accepted. CI still lacks the Apple certificate/password and App Store Connect notary key/ID/issuer secrets. |
