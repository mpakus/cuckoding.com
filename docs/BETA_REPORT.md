...
| Signed and notarized build identified | pending fresh build | Local Developer ID identity, `Cuckoding` notary profile, private updater key file, and GitHub Releases public URLs are present. The retained signed ZIP's provenance names source `4a2cf53`, not current main. Local release variables are not set in this shell; CI lacks the Apple certificate/password and App Store Connect notary key/ID/issuer secrets. Preserve the older artifact until a fresh accepted-revision build is ready. |
