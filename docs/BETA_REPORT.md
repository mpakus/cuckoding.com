...
| Release CI source gate | configured; run pending | The release workflow places `mix quality` before certificate import and notary/updater secrets. Local source quality passed at `6a4cd90` (10 properties, 285 tests); the frozen signed candidate has no matching CI run yet. |
