...
| F-1003-002 | High | The replacement onboarding form still combined project registration, agent configuration, first board/task, run, branch, and worktree creation; it did not present existing projects as the primary product object | Sole stakeholder review and screenshot, 2026-09-18; former `lib/cuckoding_web/live/status_live.ex` `guided-run-form` | closed locally; board-creation tranche remains tracked in Phase 10 | codex | ADR-023 defines the project-first boundary; `/` lists durable projects and resources, `/projects/new` is a three-step non-executing registration wizard, and `/projects/:id/edit` versions multiple agent connections and role assignments without rewriting board/run snapshots |
