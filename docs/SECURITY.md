...
- Codex credentials use the operating-system Keychain through `cli_auth_credentials_store = "keyring"` and may be authorized once from an owner-only application data directory. Every run still receives a separate `CODEX_HOME`, configuration, history, instructions, and schema; only the Keychain credential is reusable. Cuckoding never copies `auth.json`, exposes the real home, or passes an API key to the agent subprocess.
