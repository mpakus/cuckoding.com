...
- Saved Codex accounts select operating-system Keychain storage through `cli_auth_credentials_store = "keyring"`. Login uses an account-owned application-data home, while each run gets a separate `CODEX_HOME`. Credential lookup across these homes is not yet demonstrated by real-provider evidence; launch must fail closed if its own probe is unauthenticated. Legacy snapshots use provider-owned run-scoped login. Cuckoding never copies `auth.json`, exposes the real home, or passes an API key to the agent subprocess.
