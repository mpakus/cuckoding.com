...
- Board task-intake runs narrow the runtime grant to read-only planning, deny network/write/shell tools, and accept only bounded structured proposals. The host validates every cited path against the owned worktree and does not promote provider text into a command or task until human import.
