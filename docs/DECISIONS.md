...
- **Cursor ACP:** `agent acp` through `AgentDesk.Providers.ACP.Client`, or `cursor agent acp` when only the editor CLI from Cursor.app is present. Handshake is sequential (`initialize` with `clientCapabilities`, optional `authenticate`, then `session/new` with MCP servers). User prompts queue until the session is ready. `fs/read_text_file` is worktree-bounded; writes are disabled. Shared framing; Cursor owns extensions. `steer_active_turn` is false.
- **Per-session MCP injection:** `AgentDesk.Providers.MCPInjection` writes `mcp.json` under the session directory in application-data and passes those servers on ACP `session/new`. It never edits global provider config.
...
- **MCP prototype:** `AgentDesk.MCP.Protocol` implements JSON-RPC `initialize`, `tools/list`, and `tools/call` for hub, lease, handoff, search, and memory tools.
...
- **Git worktrees:** App-owned worktrees under `Storage.worktree_dir/2`, branch `agentdesk/<session_id>`, cleanup only when app-owned, path-matched, linked, and not dirty. Empty repositories start an isolated worktree from an empty commit on the agent branch only; they do not require a commit on the user's current branch.
