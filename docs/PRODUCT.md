...
- A machine-wide saved-agent catalog plus project role defaults. The built-in roles are Specifications, Coding, and Review; users may attach the same saved agent to several projects, add roles, and edit role instructions and output contracts. Cuckoding stores connection metadata and authorization status, never provider credentials.
...
4. Confirming the wizard registers only the project and redirects to its settings page. There the user can add or reuse multiple saved agents, authorize supported runtimes once, edit or add roles, and assign one connection to each role. Every project save creates an immutable trusted configuration revision; it does not create or start work.
...
7. The user refines a Draft task and marks it Ready. **Prepare run** snapshots the workflow, role assignments, policy, and saved-agent references, then creates the feature branch and owned worktree. Before launch, the run page verifies authorization without placing credential values in its snapshot or runtime directory.
