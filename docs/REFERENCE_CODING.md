...
The verified baseline is XERJ `v1.0.0-rc.74` in lexical mode. Lexical is the upstream default; do not call it neural or semantic retrieval. The current project index is `cuckoding-project-v5`; v4 remains frozen at the initial task 0004 spike schema. A future neural index requires an explicit model, a distinct prefix, measured benefit, and updated evidence.
...
```sh
rtk xerj autoindex . --dry-run --no-graph --prefix cuckoding-project-v5 --state-dir ~/.local/share/cuckoding/xerj/autoindex/cuckoding-project-v5 --progress plain
rtk xerj autoindex . --no-graph --prefix cuckoding-project-v5 --state-dir ~/.local/share/cuckoding/xerj/autoindex/cuckoding-project-v5 --progress plain --yes
```
...
```sh
rtk xerj search --prefix cuckoding-project-v5 -k 5 "event before broadcast durable state"
