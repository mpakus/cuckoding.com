...
The verified baseline is XERJ `v1.0.0-rc.74` in lexical mode. Lexical is the upstream default; do not call it neural or semantic retrieval. A future neural index requires an explicit model, a distinct prefix, measured benefit, and updated evidence.
...
```sh
rtk xerj autoindex . --dry-run --no-graph --prefix cuckoding-project-v2 --state-dir ~/.local/share/cuckoding/xerj/autoindex/cuckoding-project-v2 --progress plain
rtk xerj autoindex . --no-graph --prefix cuckoding-project-v2 --state-dir ~/.local/share/cuckoding/xerj/autoindex/cuckoding-project-v2 --progress plain --yes
```
...
```sh
rtk xerj search --prefix cuckoding-project-v2 -k 5 "event before broadcast durable state"
