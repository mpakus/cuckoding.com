...
- UTC timestamps with microsecond precision; durations as integer milliseconds, stored twice where relevant: `active_ms` (monotonic) and `wall_ms`.
...
| `metric_rollups` | `scope_type`, `scope_id`, `window`, `metric`, `value`, `unit`, `computed_at` | Disposable projections |
