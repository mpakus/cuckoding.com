...
update_archive="$dist/Cuckoding-$version-macos-arm64.app.tar.gz"
...
cleanup() { rtk proxy rm -f "$notary_archive" ${temporary_updater_key:+"$temporary_updater_key"}; }
...
echo "Updater artifact: $update_archive"
